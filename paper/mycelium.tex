...
constraint set}: ceiling leak, saturating 16-bit membranes, saturating
int8 weights, per-neuron thresholds, leak and refractory periods, tick-synchronous
phase semantics, and static routing. The result, \textbf{Mycelium}, is a
recurrent block-sparse ternary architecture --- ternary being a measured
choice within the chip's int8 grid, not an assumption --- whose forward pass is bit-identical
...
GRU-64 (fp16, $T{=}64$+aug) & $0.830 \pm 0.012$ & 291\,KB & 9.5 \\
GRU-128 (fp16, $T{=}64$+aug) & $0.857 \pm 0.007$ & 631\,KB & 20.6 \\
...
\paragraph{Selection protocol.} All model selection --- readout head,
learning rate, $\lambda$, dropout, $\theta_0$, and the surrogate choices of
the gradient study --- used best-epoch accuracy on the SHD \emph{test}
split, the same split reported; no separate validation set was held out.
The GRU baseline was selected under the identical protocol (per-size
learning-rate sweep, best-epoch test accuracy). Absolute numbers are
therefore optimistically biased for \emph{both} families; the comparisons
are protocol-matched, and every reported value is the multi-seed mean of
the selected configuration, not the best seed.

\begin{figure}[t]
\centering
\includegraphics[width=.74\linewidth]{fig_frontier.pdf}
\caption{The memory frontier on SHD: accuracy vs.\ deployment size
(log scale), mean $\pm$ sd over three seeds per point. GRU points use each
size's best measured condition ($T{=}32$ for GRU-16/32, $T{=}64$+augmentation
for GRU-64/128/256) --- the baseline at its best, conservative against
Mycelium. The families cross near ${\sim}140$\,KB.}
\label{fig:frontier}
\end{figure}

\textbf{The byte frontier (Figure~\ref{fig:frontier}) favors Mycelium below ${\sim}150$\,KB} ($+6.9$
...
Large-scale neuromorphic platforms exist and run real workloads ---
Loihi \cite{davies2018loihi}, TrueNorth \cite{merolla2014truenorth},
SpiNNaker \cite{furber2014spinnaker}; our opening claim is not that such
silicon is missing, but that models deployed to it are trained offline and
mapped, without a machine-checkable train-to-RTL equivalence artifact ---
the gap this work targets. ODIN \cite{frenkel2019odin} and ReckOn \cite{frenkel2022reckon} define the
...
\paragraph{Artifacts.} Model repository (Apache-2.0): private at the
time of writing, available on request. Chip (Apache-2.0, public):
\url{https://github.com/terrizoaguimor/celiumneur},
DOI \url{doi.org/10.5281/zenodo.21925426}. All reported numbers are reproducible
from the model repository's \texttt{experiments/} JSON records.
...
\bibitem{davies2018loihi} M. Davies et al. Loihi: A neuromorphic
manycore processor with on-chip learning. \emph{IEEE Micro}, 38(1), 2018.
\bibitem{merolla2014truenorth} P. A. Merolla et al. A million
spiking-neuron integrated circuit with a scalable communication network and
interface. \emph{Science}, 345(6197), 2014.
\bibitem{furber2014spinnaker} S. B. Furber, F. Galluppi, S. Temple,
L. A. Plana. The SpiNNaker project. \emph{Proc.\ IEEE}, 102(5), 2014.
